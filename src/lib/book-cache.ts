...
export type CachedAudio = { key: string; audio: Blob; bookId?: string; cachedAt: number }
...
class JerryReadCache extends Dexie {
  ...
  constructor() {
    ...
    this.version(3).stores({ parsedBooks: 'bookId, cachedAt', audio: 'key, bookId, cachedAt', progress: 'bookId, updatedAt' }).upgrade((transaction) => transaction.table('audio').clear())
  }
}
...
export function getCachedBook(bookId: string, sourceSize: number) {
  return cache.parsedBooks.get(bookId).then((book) => (book?.sourceSize === sourceSize && book.parserVersion === 3 ? book : undefined))
}
...
export function removeCachedBook(bookId: string) {
  return Promise.all([cache.parsedBooks.delete(bookId), cache.progress.delete(bookId), cache.audio.where('bookId').equals(bookId).delete()])
}
...
export async function getCachedCovers(bookIds: string[]) {
  const books = await cache.parsedBooks.bulkGet(bookIds)
  return Object.fromEntries(books.flatMap((book) => book?.cover ? [[book.bookId, book.cover]] : [])) as Record<string, Blob>
}
